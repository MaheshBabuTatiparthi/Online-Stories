...
/** onesignal **/
$config["onesignal_app_id"]				= "35bea08b-e636-432a-976d-987a5d507912" ; //Your OneSignal AppId, available in OneSignal https://documentation.onesignal.com/docs/generate-a-google-server-api-key
$config["onesignal_api_key"]			= "OGQ0NjA0ODUtZTc1Ni00ZjM1LTkxNTktYmM1NTcxNDZkNTZl" ; //Your OneSignal ApiKey, required for push notification sender
...
switch($_GET["page"]){
	...
	case "login":
		...
		$content .= '<img src="'.$config["logo"].'?1602860175" />';
	...
	case "stories":
		...
		switch($_GET["action"]){
			case "list":
				...
				$content .= '<tbody>';
				/** fetch data from mysql **/
				$sql_query = "SELECT * FROM `stories` ORDER BY `story_id` DESC" ;
				if($result = $mysql->query($sql_query)){
					while ($data = $result->fetch_array()){
						$content .= '<tr>';
			...
			case "edit":
				...
				if(isset($_GET["id"])){
					$entry_id= (int)$_GET["id"];
					/** fetch current data **/
					$sql_query = "SELECT * FROM `stories` WHERE `story_id`=$entry_id LIMIT 0,1" ;
					$result = $mysql->query($sql_query);
					$rowdata = $result->fetch_array();
					if(isset($rowdata["story_id"])){
						/** default value **/
						...
						if(isset($_POST["submit"])){
							...
							$sql_query = "UPDATE `stories` SET `story_title` = '{$postdata["story-title"]}' ,`story_image` = '{$postdata["story-image"]}' ,`story_category` = '{$postdata["story-category"]}' ,`published_date` = '{$postdata["published-date"]}' ,`story_content` = '{$postdata["story-content"]}'  WHERE `story_id`=$entry_id" ;
							$stmt = $mysql->prepare($sql_query);
							$stmt->execute();
							$stmt->close();
						...
						if($option_result){
							while ($option_data = $option_result->fetch_array()){
								$options["story_category"][] = array("val"=> $option_data["cat_id"],"label"=>$option_data["category_name"]);
							}
						}else{
			...
			case "add":
				...
				if(isset($_POST["submit"])){
					...
					$sql_query = "INSERT INTO `stories` (`story_title`,`story_image`,`story_category`,`published_date`,`story_content`) VALUES ('{$postdata['story-title']}','{$postdata['story-image']}','{$postdata['story-category']}','{$postdata['published-date']}','{$postdata['story-content']}')" ;
					$mysql->query($sql_query);
					$last_id = $mysql->insert_id;
			...
			case "delete":
				...
				if(isset($_GET["id"])){
					...
					$rowdata = $result->fetch_array();
					if(isset($rowdata["story_id"])){
						$sql_query = "DELETE FROM `stories` WHERE `story_id`=$entry_id";
						$stmt = $mysql->prepare($sql_query);
						$stmt->execute();
						$stmt->close();
